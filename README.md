...
Encode both key and value in Base64, and output each message on a separate line in `<key>,<value>` format:

    $ kafkacat -b mybroker -t mytopic -S base64 -K,


Encode only the value in Base64, and output each message in a JSON envelope:

    $ kafkacat -b mybroker -t mytopic -S value=base64 -J
